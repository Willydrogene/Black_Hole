...
void explorationCI(double i, double b_end, double b_start, double db, double alpha_start, double daplha, double theta_start, double dtheta, double rs, double R_min, double R_max, int nb_images, FILE* file) {
...
int main(int argc, char* argv[]){
    ...
    double i = M_PI/50;               // Angle i, inclinaison du disque d'accrétion
    const char* filename = "image_detecteur.txt"; // Nom de fichier par défaut

    if (argc >= 2) {
        i = atof(argv[1]);                  // Convertit la chaîne en double
    }
    if (argc >= 3) {
        filename = argv[2];                 // Nom du fichier de sortie
    }
    ...
    FILE* file = fopen(filename, "w");      // Ouverture du fichier de sortie

    explorationCI(i, b_end, b_start, db, alpha_start, daplha, theta_start, dtheta, rs, R_min, R_max, nb_images, file);  // Appel de la fonction qui fait les calculs
