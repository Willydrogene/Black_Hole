...
d = [50, 20, 10, 6, 5, 4, 3, 2]                                 # pi / Dénominateur
angles = []                                                     # On initialise une liste d'angles
for denominateur in d: angles.append(np.pi/denominateur)        # On remplie notre liste d'angles pi/dénominateur

for indice, i_rad in enumerate(angles):                         # On boucle sur nos angles qu'on veut plot

    # ================== 1. Lancer le C pour une inclinaison i ==================

    EXEC = "./trou_noir"                                        # Chemin de l'exécutable C (adapter si besoin, par ex. "trou_noir.exe" sous Windows)

    
    txt_file = f"image_detecteur_pi_{d[indice]}.txt"            # Nom du fichier texte produit par le C pour cette inclinaison

    print(f"Lancement du code C pour i = pi/{d[indice]}...")
    
    subprocess.run([EXEC, str(i_rad), txt_file], check=True)    # On appelle le programme C : argv[1] = i, argv[2] = nom fichier
    print(f"Calcul terminé, données écrites dans {txt_file}")

    # ================== 2. Lecture des données produites par le C ==================
    ...
    # ========================= 4. initialisation de l'histogramme des fréquences =========================
