...
SOURCE_IMAGE = "fond6.png"          # Image de fond (doit être dans le même dossier)
OUT_PNG = "image_lentille_fond6.png"      # Sortie enregistrée

b_max = 50.0                        # La taille du détecteur dans le C est à 100*rs/2 (demi détecteur), format carré
...
ybg_min, ybg_max = -100.0, 100.0    # Si on met trop petit, le trou noir apparaît comme un trèfle, parce que les rayons proches vont chercher des pixels en dehors de l'image, donc noir
...
with open(BIN_FILE, "rb") as f:         # Ouverture du fichier binaire en lecture ("rb" = read binary). Le streaming permet de traiter des fichiers très volumineux sans les charger en RAM.
    ...
    while True:
        ...
        r = np.sqrt(x_det**2 + y_det**2)        # Distance radiale au centre du détecteur, utilisée pour normaliser la direction du rayon
        ux = x_det/r                            # Composante x du vecteur directionnel unitaire dans le plan transverse
        uy = y_det/r                            # Composante y du vecteur directionnel unitaire dans le plan transverse

        # --- coordonnées sur le plan du fond
        x_bg = y_local * ux                     # Coordonnée X de l’impact sur le plan de fond après projection dans le repère global
        y_bg = y_local * uy                     # Coordonnée Y de l’impact sur le plan de fond après projection dans le repère global
