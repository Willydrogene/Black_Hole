...
void explorationCI(double x_start, double x_end, double dx, double y_start, double y_end, double dy, double theta_start, double dtheta, double rs, double distance_bg) {
    ...
    #pragma omp parallel                                // Début de la région parallèle : les threads OpenMP sont lancés ici
    {
        ...
        for (int step = 0; step < n_steps; ++step) {                    // step est privé à chaque thread, géré par OpenMP
            double x = x_start + step * dx;                             // On reconstruit x à partir de step
            ...
            for(double y = y_start ; y<y_end ; y+=dy){                               // Boucle sur y (quart d'image), (séquentielle à l'intérieur de chaque thread)                     
                double b = sqrt(x*x+y*y);                                                                       // On calcule b à cahque fois vu qu'on est en (x,y)
                ...
                double y_local=0;                                                                               // On initialise y à 0
                double theta = theta_start;                                                                     // On initialise theta à notre valeur de départ (normalement 0.0)

                double r = RK4(&u, &u_point, dtheta, rs, &y_local, distance_bg, &theta);                              // On récupère la dernière valeur de r après avoir résolu l'equa diff 
                ...
                if(r>rs && theta<3.0*M_PI/2.0){                                                         		    // Condition pour savoir si on n'est pas dans le trou noir, et si qu'on n'a pas fait un tour de trou noir (peu visible)
                    ...
                    #pragma omp critical(file_write)                                                                // Section critique: un seul thread à la fois peut écrire dans le fichier
                    {
                        ...
                        // 1er quart d'image
                        rec[0] = (float)x;                                                                          // On met x en float pour gagner de l'espace, et on le met dans la premiere valeur du tableau
                        rec[1] = (float)y;                                                                          // Pareil avec y
                        rec[2] = (float)y_local;                                                                    // Pareil avec y_local
                        fwrite(rec, sizeof(float), 3, file);                                                        // On écrit dans le fichier notre tableau (c'est du binaire donc pas de fprintf)

                        // 2eme quart d'image
                        rec[0] = (float)x;                                                                          
                        rec[1] = (float)-y;
                        rec[2] = (float)y_local;                        // En fait, on a une symétrie sur x mais aussi sur y, car pas de disque d'accrétion ici, seulement une lentille
...
int main(){
    ...
    const double x_end = 50*rs;             // Dernière valeur de x (on arrête x à cette valeur), c'est la taille du détecteur
    const double dx = 1e-2;                 // Le pas de x, ce qui va déterminer le nombre d'élément x, + il est petit, + les calculs sont longs
    
    const double y_start = 0.0;             // Première valeur de y du détecteur
    const double y_end = 50*rs;             // Dernière valeur de y (on arrête y à cette valeur), c'est la taille du détecteur
    ...
    explorationCI(x_start, x_end, dx, y_start, y_end, dy, theta_start, dtheta, rs, distance_bg);  // Appel de la fonction qui fait les calculs
